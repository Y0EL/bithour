...
export default function InvoicesPage() {
        ...
        return (
            <div style={{ textAlign: 'center', padding: '100px 20px', background: '#fff', borderRadius: '32px', border: '1px solid #f1f5f9', marginTop: '40px' }}>
                ...
                <p style={{ color: '#64748b', fontSize: '16px', maxWidth: '400px', margin: '0 auto 32px' }}>
                    Role <b>Analyst</b> tidak memiliki izin untuk melihat manajemen invoice.
                </p>
            </div>
        );
    }
    ...
    const [modal, setModal] = useState<{ open: boolean; title: string; message: string; type: 'success' | 'error' | 'info' | 'confirm' }>({
        open: false,
        title: '',
        message: '',
        type: 'info'
    ...
    const fetchDocuments = async () => {
        setLoading(true);
        try {
            const res = await fetch('/api/documents/tracking?type=INVOICE');
    ...
    const getUsername = (doc: any) => {
        const ref = doc.docNo;
        const meta = doc.metadata || {};
        const username = meta.from_username || meta.username;
        if (username) return username.startsWith('@') ? username : `@${username}`;
        if (ref && ref.startsWith('INV_')) {
    ...
    return (
        <div style={{ padding: '0px', maxWidth: '1200px', margin: '0 auto' }}>
            <div style={{ padding: '0 16px', marginBottom: '24px' }}>
                <h1 style={{ fontSize: 'clamp(24px, 5vw, 32px)', fontWeight: 800, marginBottom: '4px', color: '#111' }}>Invoices</h1>
                <p style={{ color: '#666', fontSize: '14px' }}>Track documents from drafting to signature</p>
            </div>
                                            ...
                                            <td style={{ padding: '10px 16px', textAlign: 'right' }}>
                                                <div style={{ display: 'flex', gap: '4px', justifyContent: 'flex-end' }}>
                                                    {doc.status !== 'CANCELLED' && (
                                                        <button
                                                            onClick={() => handleDownload(getCleanUrl(doc.fileUrl), (doc.fileUrl.split('/').pop() || 'invoice.pdf'), doc.id)}
                                                            className="btn-icon"
